...
export default function Config(props: ConfigsProps) {
    ...
    useEffect(() => {
        setComputeNow(false)
    }, [featureName, outliers_mode, labelFeatureName])



    useEffect(() => {
        if (featureName && datasetName && props.metricName == "completeness") {
    ...
    const handleClickCompute = async () => {
        ...
        const isDuplicate = reportToSave?.metrics?.some((metricReport: any) =>
            (
        ...
        if (!hasValidationErrors && !isDuplicate) {
            ...
            try {
                const results = await waitForActionResult();  //the variable "results" gets populated inside the AsyncTracker component. This awaits for it to be populated.
    ...
    const handleSaveToReport = () => {
        ...
        if (reportMetric) {
            ...
            console.log("reportMetric to add", reportMetric)


            
            console.log("current report to save LOCALLY", reportToSave)

            setReportToSave([...reportToSave, reportMetric.results]);

            saveMetricToReport(datasetName as string, [...reportToSave, reportMetric]);
    ...
    const metricDisplayerMap: Record<string, React.ComponentType> = {
        ...
        "completeness": () => <CompletenessDisplayer completeness={completeness as CompletenessDTO} requirements={requirements} />
    };
                                ...
                                <div style={{ marginBottom: "15px" }}>
                                    <AlertCust
                                        ...
                                        textToDisplay={<>
                                            ...
                                            the dedicated page <Link href="/pages/tasks/dataquality/actions/embeddings?autoSelectModel=true">here</Link>
                                        </>} />
                                </div>
                            </>
                        )}
                        ...
                        (<>
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        <Modal.Title style={{ fontWeight: 700, fontSize: "1.25rem", color: "var(--bg-dark)", display: "flex", alignItems: "center" }}>
                                    ...
                                    <Modal.Body style={{ padding: "20px", backgroundColor: "white", }}>
